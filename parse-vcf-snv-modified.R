...
parse.snv = function(sample.dir) {
  ...
  # Features to include
  # mutect2.features = c('CONTQ', 'ClippingRankSum', 'DP', 'ECNT', 'FS', 'GERMQ', 'MQ', 'MQ0', 'MQRankSum', 'ReadPosRankSum', 'SEQQ', 'STRANDQ', 'DB')
  # freebayes.features = c('AN', 'DP', 'DPB', 'EPPR', 'GTI', 'MQMR', 'NS', 'NUMALT', 'ODDS', 'PAIREDR', 'PQR', 'PRO', 'QR', 'RO', 'RPPR', 'SRF', 'SRP', 'SRR', 'DB')
  # varscan.features = c('DP', 'GPV', 'SPV', 'SS', 'SSC', 'DB')
  # vardict.features = c('DB', 'DP', 'MSI', 'MSILEN', 'SAMPLE', 'SHIFT3', 'SOR', 'SSF', 'STATUS', 'TYPE', 'VD')

  mutect2.features = c('ClippingRankSum', 'DP', 'ECNT', 'FS', 'MQ', 'MQ0', 'MQRankSum', 'ReadPosRankSum')
  freebayes.features = c('AN', 'DP', 'DPB', 'EPPR', 'GTI', 'MQMR', 'NS', 'NUMALT', 'ODDS', 'PAIREDR', 'PQR', 'PRO', 'QR', 'RO', 'RPPR', 'SRF', 'SRP', 'SRR')
  varscan.features = c('DP', 'GPV', 'SPV', 'SS', 'SSC') 
  vardict.features = c('DP', 'MSI', 'MSILEN', 'SAMPLE', 'SHIFT3', 'SOR', 'SSF', 'STATUS', 'TYPE', 'VD')
  ...
  pass_m2<- vcf_m2@fixed$FILTER=="PASS" | vcf_m2@fixed$FILTER=="MinAF" | !is.na(vcf_m2@fixed$FILTER)
  pass_f<- vcf_f@fixed$FILTER=="PASS" | !is.na(vcf_f@fixed$FILTER)
  pass_vs<- vcf_vs@fixed$FILTER=="PASS" | !is.na(vcf_vs@fixed$FILTER)
  pass_vd<- vcf_vd@fixed$FILTER=="PASS" | !is.na(vcf_vd@fixed$FILTER)
  ...
  meta_data$FILTER_Mutect2_data <- meta_data$FILTER_Mutect2
  ...
  meta_data$FILTER_Freebayes_data <- meta_data$FILTER_Freebayes
  ...
  meta_data$FILTER_Vardict_data <- meta_data$FILTER_Vardict
  ...
  meta_data$FILTER_Varscan_data <- meta_data$FILTER_Varscan
  ...
  parse_snv <- meta_data[,c("seqnames","start","end","REF","ALT","REF_MFVdVs","ALT_MFVdVs","Sample_Name",
                            "FILTER_Mutect2","FILTER_Freebayes","FILTER_Vardict","FILTER_Varscan", "FILTER_Mutect2_data","FILTER_Freebayes_data","FILTER_Vardict_data","FILTER_Varscan_data",
                            names.features)]
...
write.table(parse.df, 'snv-parse-real1.txt', row.names = F, quote = F, sep = '\t')
print("real1 done")

parse.df = parse.snv(sample.dir='real2_part1')
write.table(parse.df, 'snv-parse-real2_part1.txt', row.names = F, quote = F, sep = '\t')
print("real2_part1 done")

parse.df = parse.snv(sample.dir='real2_part2')
write.table(parse.df, 'snv-parse-real2_part2.txt', row.names = F, quote = F, sep = '\t')
print("real2_part2 done")

parse.df = parse.snv(sample.dir='syn1')
write.table(parse.df, 'snv-parse-syn1.txt', row.names = F, quote = F, sep = '\t')
print("syn1 done")

parse.df = parse.snv(sample.dir='syn2')
write.table(parse.df, 'snv-parse-syn2.txt', row.names = F, quote = F, sep = '\t')
print("syn2 done")

parse.df = parse.snv(sample.dir='syn3')
write.table(parse.df, 'snv-parse-syn3.txt', row.names = F, quote = F, sep = '\t')
print("syn3 done")
